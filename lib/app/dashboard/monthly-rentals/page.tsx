  ...
  const exportRows =
    enrichedRentals.map(
      (item: any) => ({
        ...
        paid_months:
          item._paid_months_text ||
          '-',

        next_payment_month:
          item._next_payment_month ||
          '-',
    ...
    <div className={ui.monthlyPage}>
      ...
      >
        <div>
          ...
          >
            管理各停車場月租戶。正式租期、已繳月份與續租提醒已分離；總表匯入不再覆蓋既有租期。已退租與到期超過 4 個月的資料不顯示於總表。
          </p>
        </div>
            ...
            >
              ...
              <thead>
                ...
                >
                  ...
                  <th style={{ padding: 8 }}>已繳月份</th>
                  <th style={{ padding: 8 }}>下次應繳</th>
                  ...
                  (item: any) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td style={{ padding: 8, fontSize: 13, lineHeight: 1.55 }}>
                          {item._paid_months_text}
                          {item._paid_months?.length === 0 && item.payment_status === 'paid' && (
                            <div style={{ color: '#b45309', marginTop: 3 }}>舊資料尚未指定月份</div>
                          )}
                        </td>

                        <td style={{ padding: 8, fontSize: 13, fontWeight: 700, color: item._next_payment_month === '本租期已繳清' ? '#15803d' : '#334155' }}>
                          {item._next_payment_month}
                        </td>
