...
export default function AttendanceUploadClient({parkingLots,defaultParkingLotId}:{parkingLots:Lot[];defaultParkingLotId?:string}){
 ...
 async function upload(){ if(!lot||!month||!file){setMessage('請選擇停車場、月份與檔案。');return}; setBusy(true);setMessage('正在上傳…'); try{ const fd=new FormData();fd.set('parkingLotId',lot);fd.set('attendanceMonth',month);fd.set('file',file); const r=await fetch('/api/attendance/upload',{method:'POST',body:fd}); const j=await r.json(); if(!r.ok) throw new Error(j.error||'上傳失敗'); setMessage('簽到表上傳完成。每次上傳都會獨立保留，不會取代同停車場同月份其他管理員的檔案。');setFile(null); const input=document.getElementById('attendance-file') as HTMLInputElement|null;if(input)input.value='';await load() }catch(e:any){setMessage(`簽到表上傳失敗：${e?.message||'未知錯誤'}`)}finally{setBusy(false)} }
 return <div style={{paddingBottom:40}}><h1>簽到表上傳</h1><p className="muted">簽到表由伺服器安全上傳；同一停車場同月份可由多位管理員分別上傳，每一份都會獨立保留。</p>
 ...
 <button type="button" className="btn" onClick={upload} disabled={busy||!file} style={{marginTop:16}}>{busy?'上傳中…':'上傳簽到表'}</button>
